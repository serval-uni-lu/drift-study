...
window_size_2 = {"lcld_201317_ds_time": 20000, "electricity": 2000}
...
def compute_distance(scaler, x, x_adv, y, y_pred):
...
def run(
    ...
    plot_scatter_correlation(
        batch_distance,
        batch_metric,
        path=f"distance_mcc_{dataset_name}_{model_name}_{'adv' if adv_training else 'clean'}",
    ...
    plot_scatter_correlation(
        delta_distance,
        delta_metric,
        path=f"distance_mcc_delta_{dataset_name}_{model_name}_{'adv' if adv_training else 'clean'}",
        metric_name="Delta Distance (L2)",
        test_name="Delta MCC",
    )

    batch_distance_2 = compute_batch_distance(
        distance, batch_size=window_size_2[dataset_name]
    )
    batch_metric_2 = compute_batch_metric(
        model,
        create_metric("mcc"),
        x_test[:N_ATTACKS],
        y_test[:N_ATTACKS],
        batch_size=window_size_2[dataset_name],
    )
    delta_distance_2 = batch_distance_2[1:] - batch_distance_2[:-1]
    delta_metric_2 = batch_metric_2[1:] - batch_metric_2[:-1]
    plot_coevolution(
        [batch_metric_2, batch_distance_2],
        path=f"evolution_distance_{dataset_name}_{model_name}_{'adv' if adv_training else 'clean'}",
        metric_names=["MCC", "Distance (L2)"],
    )
    plot_coevolution(
        [delta_metric_2, delta_distance_2],
        path=f"evolution_distance_delta_{dataset_name}_{model_name}_{'adv' if adv_training else 'clean'}",
