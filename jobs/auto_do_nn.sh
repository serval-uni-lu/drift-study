...
if [ "$stage" = "all" ] || [ "$stage" = 3 ]; then
    echo "Periodic"
    python -m drift_study.run.pretrain_models -c config/logging.yaml -c ${AUTO_CONFIG} -p pretrain_periodic=true -p use_auto_model_tuning=true -p train_window_sizes=${DEFAULT_WINDOW_SIZE}
    echo "Evaluate periodic"
    python -m drift_study.run.periodic_baseline -c config/logging.yaml -c ${AUTO_CONFIG} -p use_auto_model_tuning=true -p schedule_data_path=${PATH_PREFIX}/periodic/ -c ./config/auto/delays_none.yaml -p train_window_size=${DEFAULT_WINDOW_SIZE}
    python -m drift_study.visualization.auto_table -c config/logging.yaml -c ${AUTO_CONFIG} -p schedule_data_path=${PATH_PREFIX}/periodic -p out_path=${PATH_PREFIX}/periodic.csv -p train_window_sizes=${DEFAULT_WINDOW_SIZE}
    if [ -z "$RETRAIN_OPT_SEARCH" ]; then
        echo "Periodic with re-train optimization"
        python -m drift_study.run.periodic_baseline -c config/logging.yaml -c ${AUTO_CONFIG} -p model.optimize=true -p schedule_data_path=${PATH_PREFIX}/opt_periodic/ -c ./config/auto/delays_none.yaml -p train_window_size=${DEFAULT_WINDOW_SIZE}
        python -m drift_study.visualization.auto_table -c config/logging.yaml -c ${AUTO_CONFIG} -p schedule_data_path=${PATH_PREFIX}/opt_periodic -p out_path=${PATH_PREFIX}/opt_periodic.csv -p train_window_sizes=${DEFAULT_WINDOW_SIZE}
    fi
fi
