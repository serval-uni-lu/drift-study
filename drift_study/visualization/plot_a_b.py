...
def filter_extreme(df, max_n_train, max_pareto_rank=1):
    filter_baseline = df["schedule_type"] == "Baseline"
    filter_others = (df["n_train"] > 1) & (df["n_train"] < max_n_train) & (df["pareto_rank"] <= max_pareto_rank)
    df = df[filter_baseline | filter_others]
    return df
